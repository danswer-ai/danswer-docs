...
export SSL_ROOT_CERT="/app/bundle.pem"
```

Ensure that `SSL_ROOT_CERT` points to the path where the certificate is mounted inside the container (`/app/bundle.pem` for Docker Compose or `/app/certs` for Kubernetes).
