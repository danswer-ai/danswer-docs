...
2. Navigate to the Kubernetes configuration directory:
   ```shell
   cd path/to/danswer/deployment/kubernetes
   ```
...
5. Navigate to the URL provided by the previous command to access Danswer.

Note: Before applying the Kubernetes configurations, you need to set up the EBS CSI driver and create an EBS StorageClass. Here's a brief explanation of the StorageClass setup:

1. The StorageClass is a Kubernetes resource that defines how storage is provisioned dynamically.
2. For AWS EKS, we typically use the EBS CSI driver to provision EBS volumes.
3. A basic EBS StorageClass configuration looks like this:
   ...
   This configuration creates a StorageClass named 'ebs-sc' that uses the EBS CSI driver to provision gp3 EBS volumes.

4. After creating this StorageClass, you can reference it in your PersistentVolumeClaims to dynamically provision EBS volumes for your pods.

Ensure you have set up the EBS CSI driver and created an appropriate StorageClass before proceeding with the Danswer deployment.
